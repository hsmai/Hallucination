...
**실행 방식 요약** (2026-07-16 확정 정책):
- **모든 GPU 작업 = python-qsub 배치** (`qsub -v PAYLOAD="..." scripts/pbs_job.sh`). **interactive 금지.**
- **새 CUDA 설치 금지** — 복제한 env가 자체 CUDA 런타임(cu121)을 갖고 있어 설치할 일 없음.
  pip/conda로 cudatoolkit·cuda 계열 패키지를 새로 설치하지 말 것.
- 노드 고정: **pleiades3, GPU 1장** (템플릿 기본값과 일치)
...
#    ⚠ interactive 금지 (2026-07-16 정책) — python-qsub 배치로 제출:
cd ~/Hallucination
cat > /tmp/precheck.sh <<'EOF'
source ~/.bashrc
cd ~/Hallucination
...
EOF
qsub -v PAYLOAD=/tmp/precheck.sh scripts/pbs_job.sh
qstat && sleep 60 && cat results/precheck/runs/avhbench/*.jsonl 2>/dev/null | head -1
```
